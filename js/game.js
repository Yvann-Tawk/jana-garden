...
function beginGame(){
  ...
  setTimeout(()=>{ el.style.display='none'; state=ST.PLAY; if(fireSnd)fireSnd.play(); }, 1200);
}
...
class Base extends Phaser.Scene {
  ...
  preloadCommon(){
    ...
    if(!this.cache.audio.has('bgm')) this.load.audio('bgm','assets/audio/bgm.wav');
    if(!this.cache.audio.has('fire')) this.load.audio('fire','assets/audio/fire.ogg');
    if(!this.cache.audio.has('flower')) this.load.audio('flower','assets/audio/flower.wav');
    if(!this.cache.audio.has('talk')) this.load.audio('talk','assets/audio/talk.wav');
...
class Cabin extends Base {
  ...
  create(){
    ...
    this.solid=ci.solid; this.door=ci.door;
